...
class LogicMenu:
    ...
    def show_interface(background, heroes_lives, level):
        ...
        for i in range(len(heroes_lives)):
            ViewBackground.blit_font(background, font_menu, heroes_lives[i], 1, (0, 0, 0),
                                         background.get_width()//2-200, 10+50*i)
    ...
    def get_settings_points_menu(settings):
        values=[]
        keys = {'space': K_SPACE, 'tab': K_TAB, 'right ctrl': K_RCTRL, 'left ctrl': K_LCTRL, 'up': K_UP, 'down': K_DOWN,
            'left': K_LEFT, 'right': K_RIGHT, 'delete': K_DELETE, 'backspace': K_BACKSPACE, 'f1': K_F1, 'f2': K_F2,
            'f3': K_F3, 'f4': K_F4, 'f5': K_F5, 'f6': K_F6, 'f7': K_F7, 'f8': K_F8, 'f9': K_F9, 'f10': K_F10,
            'f11': K_F11, 'f12': K_F12, 'f13': K_F13, 'f14': K_F14, 'caps lock': K_CAPSLOCK, 'right shift': K_RSHIFT,
            'left shift': K_LSHIFT, 'left alt': K_LALT, 'right alt': K_RALT, 'menu': K_MENU, 'insert': K_INSERT,
            'print screen': K_PRINT, 'end': K_END, 'page up': K_PAGEUP, 'page down': K_PAGEDOWN, 'pause': K_PAUSE,
            '[0]': K_KP0, '[1]': K_KP1, '[2]': K_KP2, '[3]': K_KP3, '[4]': K_KP4, '[5]': K_KP5, '[6]': K_KP6,
            '[7]': K_KP7, '[8]': K_KP8, '[9]': K_KP9, 'enter': K_KP_ENTER, '[/]': K_KP_DIVIDE,  '[+]': K_KP_PLUS,
            '[-]': K_KP_MINUS, '[*]': K_KP_MULTIPLY, '[.]': K_KP_PERIOD, 'numlock': K_NUMLOCK}
        items = keys.items()
        for s in settings:
            for k, v in items:
                if v == s:
                    ...
                    values.append(chr(s))
        return [[50, 380, u'Back', 1], [250, 100, u'%s' % values[0], 2], [250, 170, u'%s' % values[1], 3],
                [250, 240, u'%s' % values[2], 4], [250, 310, u'%s' % values[3], 5], [550, 100, u'%s' % values[4], 6],
                [550, 170, u'%s' % values[5], 7], [550, 240, u'%s' % values[6], 8], [550, 310, u'%s' % values[7], 9],
                [50, 100, u'Up', 0], [50, 170, u'Left', 0], [50, 240, u'Right', 0], [50, 310, u'Fire', 0],
                [250, 30, u'1 Player', 0], [550, 30, u'2 Player', 0], [50, 100, u'Up', 0], [50, 170, u'Left', 0],
                [50, 240, u'Right', 0], [50, 490, u'To change button press Enter', 0],
                [50, 540, u'To install selected button', 0], [50, 580, u'press Escape', 0]]
